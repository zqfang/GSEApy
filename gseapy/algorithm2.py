...
def enrichment_score(gene_list, gene_set, weighted_score_type=1, correl_vector=None,
                     esnull=None, permutation_type=None, rs=np.random.RandomState()):
    ...
    :param permutation_type: Permutation type, "phenotype" (default) for phenotypes, "gene_set" for genes. Only use this
                             when esnull is not None.                          
...
def ranking_metric(df, method, phenoPos, phenoNeg, classes, ascending, rs=None):
   ...
   :param rs: random state. Default: np.random.RandomState().
   :return: returns correlation to class of each variable. same format with .rnk file. gene_name in first coloum,
    ...
    df2 = df.copy().T   
    if rs:
        df2 = df2.apply(rs.shuffle, axis=0)
...
def gsea_compute(data, gmt, n, weighted_score_type, permutation_type, method,
    ...
    if permutation_type == "phenotype":
        print("Permutation type equals to phenotype...............\n"+
              "This step might take a while to run. Be patient...")
        ranking_null = np.empty((n, len(ranking)))
         
        for i in range(n):
            #permutation classes
            r2 = ranking_metric(df=data, method=method, phenoPos=phenoPos, phenoNeg=phenoNeg, 
                                classes=classes, ascending=ascending, rs=rs)
            ranking_null[i] = r2['rank'].values
          
        for si,subset in enumerate(subsets):
            esn = enrichment_score(gene_list=gene_list, gene_set=gmt.get(subset), permutation_type=permutation_type,
                                   esnull=n, weighted_score_type=w, correl_vector=ranking)[0] 
